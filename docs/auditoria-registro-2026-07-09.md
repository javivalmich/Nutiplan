...
| EDITORIAL-D5 | No tiene entrada propia | `scripts/phaseB2/output/dishCompositionConfirmations.json` (241 platos, 419 campos confirmados, verificado en vivo); `src/engine2/dishes/compositionResolver.js:217,225-228,247,253-256` (consumo de `fuenteEditorial.confirmed`) | Sesión editorial humana con el cocinero, confirmada factualmente por el propietario; sin commit ni PR propio por ser sesión humana, pero D5 es quien produjo las 419 confirmaciones que D6 ingirió — la huella de D5 es la misma que la de D6, no una ausencia | SIN ASIENTO, EN USO |
